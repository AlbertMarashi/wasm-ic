...
use veryl_parser::{
    veryl_grammar_trait::{
        FactorTypeGroup, FirstToken, ModuleDeclaration, PortDeclarationGroupGroup,
        PortDeclarationItemGroup, ScalarTypeGroup,
    },
...
impl VerylWalker for ModuleFinder<'_, '_> {
    fn module_declaration(&mut self, module: &ModuleDeclaration) {
        ...
        if &self.source_code.as_bytes()
            [name_token.pos as usize..(name_token.pos + name_token.length) as usize]
            == self.look_for.as_bytes()
        {
            if let Some(port_declarations) = module
                .module_declaration_opt2
                .as_ref()
                .and_then(|opt2| opt2.port_declaration.port_declaration_opt.as_ref())
                .map(|opt| &opt.port_declaration_list)
            {
                let veryl_ports = iter::once(&port_declarations.port_declaration_group)
                    .chain(
                        port_declarations
                            .port_declaration_list_list
                            .iter()
                            .map(|after_first| &after_first.port_declaration_group),
                    )
                    .filter_map(|group| match &*group.port_declaration_group_group {
                        PortDeclarationGroupGroup::LBracePortDeclarationListRBrace(_) => None,
                        PortDeclarationGroupGroup::PortDeclarationItem(
                            port_declaration_group_group_port_declaration_item,
                        ) => Some(port_declaration_group_group_port_declaration_item),
                    })
                    .map(|item| {
                        let item = &item.port_declaration_item;
                        let port_name_token = item.identifier.identifier_token.token;
                        let port_name_bytes = &self.source_code.as_bytes()[port_name_token.pos
                            as usize
                            ..(port_name_token.pos + port_name_token.length) as usize];
                        let port_name_str = str::from_utf8(port_name_bytes).expect(
                            "Veryl bug: Veryl identifier had invalid byte range (invalid UTF-8)",
                        );
                ...
                for (port_name, port_type) in veryl_ports {
                    match &**port_type {
                        PortDeclarationItemGroup::PortTypeConcrete(
                            ...
                            let concrete_type =
                                &port_declaration_item_group_port_type_concrete.port_type_concrete;

                            let port_direction = match &*concrete_type.direction {
                                veryl_parser::veryl_grammar_trait::Direction::Input(_) => {
                                    PortDirection::Input
                                }
                                veryl_parser::veryl_grammar_trait::Direction::Output(_) => {
                                    PortDirection::Output
                                }
                                veryl_parser::veryl_grammar_trait::Direction::Inout(_) => {
                                    PortDirection::Inout
                                }
                            ...
                            let port_width = match &*concrete_type
                                .array_type
                                .scalar_type
                                .scalar_type_group
                            {
                                ScalarTypeGroup::UserDefinedTypeScalarTypeOpt(
                                    _scalar_type_group_user_defined_type_scalar_type_opt,
                                ) => todo!("What is UserDefinedTypeScalarTypeOpt"),
                                ScalarTypeGroup::FactorType(scalar_type_group_factor_type) => {
                                    match &*scalar_type_group_factor_type
                                        .factor_type
                                        .factor_type_group
                                    {
                                        FactorTypeGroup::VariableTypeFactorTypeOpt(
                                            factor_type_group_variable_type_factor_type_opt,
                                        ) => {
                                            if let Some(factor_type) =
                                                factor_type_group_variable_type_factor_type_opt
                                                    .factor_type_opt
                                                    .as_ref()
                                            {
                            ...
                            ports.push((port_name.to_string(), port_width, 0, port_direction));
                        }
                        PortDeclarationItemGroup::PortTypeAbstract(_) => {
                            // Abstract types include `clock` and `reset` in
                            // Veryl.  These compile down to 1-bit input wires
                            // in SystemVerilog, so expose them as such.
                            ports.push((
                                port_name.to_string(),
                                1, // 1-bit (same as logic without width)
                                0,
                                PortDirection::Input,
                            ));
                        }
...
pub fn veryl(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let Some(veryl_toml_path) = search_for_veryl_toml(manifest_directory) else {
        return syn::Error::new_spanned(args.source_path, "Could not find Veryl.toml")
            .into_compile_error()
            .into();
    };
    ...
    let source_code = match fs::read_to_string(&veryl_source_path) {
        ...
        Err(error) => {
            return syn::Error::new_spanned(
                &args.source_path,
                format!("Failed to read source code file at {veryl_source_path}: {error}"),
            )
            .into_compile_error()
            .into();
        }
    ...
    let ports = if let Some(ports) = module_finder.found {
        ...
        return module_finder
            .error
            .expect("Marlin bug for Veryl integration: ModuleFinder exited without ports or error")
            .into_compile_error()
            .into();
    };
    ...
    // Determine the .sv file path based on the build.target config.
    // - type = "source": .sv sits next to .veryl (same directory)
    // - type = "directory": .sv is in the specified target path
    let verilog_source_path = {
        let sv_filename = veryl_source_path
            .file_stem()
            .map(|s| format!("{s}.sv"))
            .unwrap_or_else(|| "unknown.sv".to_string());

        let sv_dir = if let Some(target) = veryl_toml.get("build").and_then(|b| b.get("target")) {
            ...
            match target_type {
                "directory" => {
                    let path = target
                        ...
                        .and_then(|p| p.as_str())
                        .unwrap_or("target");
                    let mut project_root = veryl_toml_path.clone();
                    project_root.pop();
                    project_root.join(path)
                }
                // "source" — same directory as the .veryl file
                _ => {
                    let mut dir = veryl_source_path.clone();
                    dir.pop();
                    dir
                }
            }
        ...
        syn::LitStr::new(sv_dir.join(&sv_filename).as_str(), args.source_path.span())
    };
